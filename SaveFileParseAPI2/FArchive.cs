...
public abstract class FArchive : Stream, ICloneable, IDisposable
{
    ...
    public void SerializeCompressedNew(byte[] dest, int length, string compressionFormatToDecodeOldV1Files, ECompressionFlags flags, bool bTreatBufferAsFileReader, out long outPartialReadLength)
    {
        ...
        if (!bReadCompressionFormat)
        {
            ...
            if (flags.HasFlag(ECompressionFlags.COMPRESS_DeprecatedFormatFlagsMask))
            {
                Console.WriteLine("Old style compression flags are being used with FAsyncCompressionChunk, please update any code using this!");
        ...
        for (var chunkIndex = 0; chunkIndex < totalChunkCount; chunkIndex++)
        {
            ...
            try
            {               
                Oodle.Decompress(compressedBuffer, 0, (int) chunk.CompressedSize, dest, destPos, (int) chunk.UncompressedSize, null);
